...
const counts = computed(() => tickets.countsFor(tickets.incidents));
const filtered = computed(() => tickets.filterAndSearch(tickets.incidents, status.value, search.value, users));
</script>
...
<template>
  <div class="space-y-5">
    <div class="card p-4 flex flex-col md:flex-row md:items-center gap-4 md:justify-between">
      <div>
        <h2 class="text-base font-semibold text-slate-900">Incident Pool</h2>
        <p class="text-xs text-slate-500">Unplanned interruptions to a service.</p>
      </div>
