...
const view = usePoolView(VIEW_KEY, () => tickets.requests);
...
const onExport = () => view.exportCsv(`requests-${today()}.csv`);
</script>
...
<template>
  <div class="space-y-5">
    <div class="card p-4 flex flex-col md:flex-row md:items-center gap-4 md:justify-between">
      <div>
        <h2 class="text-base font-semibold text-slate-900 dark:text-white">Request Pool</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400">Service catalog and standard fulfillment requests.</p>
      </div>
